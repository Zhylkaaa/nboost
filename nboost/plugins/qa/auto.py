...
class AutoQAModelPlugin(QAModelPlugin):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.logger = set_logger(self.model_dir, verbose=True)

        self.model = AutoModelForQuestionAnswering.from_pretrained(self.model_dir)
        self.logger.info('Auto Loading from checkpoint %s' % self.model_dir)
    ...
    def get_answer(self, query: str, choice: str) -> Tuple[str, int, int, float]:
        ...
        # self.logger.info(f"{answer}, {start_char_offset}, {end_char_offset}, {float(max_score)}")
